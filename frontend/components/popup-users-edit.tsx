...
export const PopupUserEdit = ({ user }: PopupUserEditProps) => {
  const [username, setUsername] = useState(user.username);
  const [email, setEmail] = useState(user.email);
  const [password, setPassword] = useState(""); // No establecer la contraseña inicialmente
  ...
  const handleEditUser = async (e: React.FormEvent) => {
    ...
    const userData = {
      ...
      username: username,
      email: email,
      // Utilizar la contraseña existente si no se proporciona una nueva
    ...
    try {
      const response = await fetch(
        `https://mate-dragons-production.up.railway.app/api/users/${user.id}`,
        {
          method: "PUT",
          headers: {
